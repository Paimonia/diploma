...
// получить список запланированных тренировок пользователя
router.get('/training/planned/:id', async (req, res) => {
  try {
    const trainingsconditions = {              
      where:  {
        user_id: req.params.id,
        start_date: { [Op.gte]: req.query.startDate, [Op.lte]: req.query.endDate }
      },
      order: [
        ['start_date', 'asc']
      ]
...
// получить список тренировок доступных для пользователя
